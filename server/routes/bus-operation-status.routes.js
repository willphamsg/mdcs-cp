...
function applyFilters(items, filter) {
  ...
  return items.filter(item => {
    ...
    const matchDepots =
      !depots?.length || depots.includes(item.depot_id);

    return matchConnection && matchAuthStatus && matchDLStatus && matchULStatus && matchDepots;
  });
}
...
router.post('/search', async (req, res) => {
  try {
    const items = await readCollection('bus-operation-status');
    let data = [...items];
...
router.post('/add', async (req, res) => {
  try {
    ...
    await replaceCollection('bus-operation-status', data);
...
router.put('/update', async (req, res) => {
  let items = await readCollection('bus-operation-status');
  ...
  await replaceCollection('bus-operation-status', merged);
...
router.delete('/delete', async (req, res) => {
  try {
    ...
    await replaceCollection('bus-operation-status', filtered);
