...
router.post('/search', async (req, res) => {
  try {
    const items = await readCollection('daily-bus-list');
    let data = [...items];
...
router.post('/add', async (req, res) => {
  try {
    ...
    await replaceCollection('daily-bus-list', data);
    ...
    //   status_code: 'INFO 3021',
    //   timestamp: Date.now(),
    //   error: 'Create new bus list failed!',
    //   payload: newItems,
    // };
...
router.put('/update', async (req, res) => {
  let items = await readCollection('daily-bus-list');
  ...
  await replaceCollection('daily-bus-list', merged);
...
router.delete('/delete', async (req, res) => {
  try {
    ...
    await replaceCollection('daily-bus-list', filtered);
